...
import { 
  ...
  Trash2, Clock, Heart, MessageCircle, Share, Link
} from 'lucide-react';
...
export const SocialPostsPage: React.FC = () => {
  ...
  const totalEngagement = socialPosts.reduce((acc, p) => 
    acc + p.engagement.likes + p.engagement.comments + p.engagement.shares, 0
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <Button variant={filter === 'facebook' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('facebook')}>
          <Facebook className="h-4 w-4 mr-1" /> Facebook
        </Button>
        <Button variant={filter === 'instagram' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('instagram')}>
          <Instagram className="h-4 w-4 mr-1" /> Instagram
        </Button>
        <Button variant={filter === 'twitter' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('twitter')}>
          <Twitter className="h-4 w-4 mr-1" /> Twitter
        </Button>
        <Button variant={filter === 'linkedin' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('linkedin')}>
          <Linkedin className="h-4 w-4 mr-1" /> LinkedIn
        </Button>
        <Button variant={filter === 'published' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('published')}>
          Published
        </Button>
        <Button variant={filter === 'scheduled' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('scheduled')}>
          Scheduled
        </Button>
            ...
            <CardContent className="p-4">
              ...
              {post.status === 'Published' && (
                <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
                  ...
                  <span className="flex items-center gap-1">
                    <Share className="h-4 w-4" /> {post.engagement.shares}
                  </span>
                  <span className="flex items-center gap-1">
                    <Link className="h-4 w-4" /> {post.engagement.clicks}
                  </span>
                </div>
              )}
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new social media post to publish
            </DialogDescription>
          </DialogHeader>
